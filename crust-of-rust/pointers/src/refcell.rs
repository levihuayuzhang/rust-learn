...
impl<T> RefCell<T> {
    ...
    pub fn borrow_mut(&self) -> Option<RefMut<'_, T>> {
        if let RefState::Unshared = self.state.get() {
            ...
            // SAFETY
            // no other reference have been given out since state would be
            // Shared or Exclusive
...
impl<T> std::ops::Deref for Ref<'_, T> {
    ...
    fn deref(&self) -> &Self::Target {
        ...
        // Once it is given out, state is set to Shared, so no
        // exclusivge references are given out.
...
impl<T> std::ops::DerefMut for RefMut<'_, T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        ...
        // So we have an exclusive lease on the inner value, so mutably derecferencing is fine.
